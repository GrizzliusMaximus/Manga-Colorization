...
line_set = "../data/line_set/"
val_set = "../data/validation_set/"
data_out = "../data/data_out/"

def GetDataset(dataset, gray ,line,num):
    for root, dirs, files in os.walk(dataset):
        for image in files:
            # print(line_set+root[num:]+"line"+image)
            temp = cv2.imread(root+image)
            temp = cv2.resize(temp, (x_shape, y_shape))
            temp = cv2.cvtColor(temp, cv2.COLOR_BGR2GRAY)
            temp = temp.reshape(temp.shape[0], temp.shape[1], 1)
            gray.append(temp)
            temp = cv2.imread(line_set+root[num:]+"line"+image)
            temp = cv2.resize(temp, (x_shape, y_shape))
            temp = cv2.cvtColor(temp, cv2.COLOR_BGR2GRAY)
            temp = temp.reshape(temp.shape[0], temp.shape[1], 1)
            line.append(temp)
...
#get dataset
GetDataset(color_set, gray, line, 18)
GetDataset(val_set, val_gray, val_line,23)

#convert to numpy array
gray = np.array(gray)
line = np.array(line)
val_gray = np.array(val_gray)
val_line = np.array(val_line)

#normalize rgb to match the output range of -1 to 1
gray = (gray-127.5)/127.5
line = (line-127.5)/127.5
val_line = (val_line-127.5)/127.5
...
while 1:
    if e%200 == 0:
        rand_indexes = np.random.randint(0, len(gray), size=batch)
        sel_gray = gray[rand_indexes]
        sel_line = line[rand_indexes]

        for i in range(len(sel_line)):
            ...
            cv2.imwrite(data_out+'batch/'+ str(e)+'/'+str(i)+'.jpg', (sel_gray[i]*127.5)+127.5) 

    print('Epoch ', e)
    # gen.fit(sel_gray,sel_rgb, epochs=1, batch_size=1)

    gen_image = gen.predict(sel_line, batch_size=16)
    inputs = np.concatenate([sel_line, sel_line])
    outputs = np.concatenate([sel_gray, gen_image])
    y = np.concatenate([np.ones((batch,1)), np.zeros((batch,1))])
    disc.fit([inputs, outputs], y, epochs=1, batch_size=4)
    disc.trainable = False
    advr.fit(sel_line, [np.ones((batch,1)),sel_gray], epochs=1, batch_size=1)
    disc.trainable = True

    if e%5 == 0:
        advr.save_weights(data_out+'updated.h5') 
    gen_image_val = (gen.predict(val_line, batch_size=8)*127.5)+127.5
